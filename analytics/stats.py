...
def categorize_missing_npcs():
    ...
    try:
        ...
        all_entities = AnalyticsService.get_all_entities()
        ...
        logger.error(f"Fehler beim Kategorisieren von NPCs: {str(e)}", exc_info=True)
        return 0
    except Exception as e:
...
def get_stats_cached(start_date_str, end_date_str, entity_filters_tuple):
    """
    Caching-Variante für get_stats. Die Parameter werden als Strings/Tuples übergeben, damit lru_cache funktioniert.
    """
    entity_filters = dict(entity_filters_tuple) if entity_filters_tuple else None
    start_date = datetime.strptime(start_date_str, "%Y-%m-%d") if start_date_str else None
    end_date = datetime.strptime(end_date_str, "%Y-%m-%d") if end_date_str else None
    return get_stats(start_date, end_date, entity_filters)
...
def get_stats(start_date=None, end_date=None, entity_filters=None):
    ...
    try:
        ...
        # Parameter-Listen für alle Abfragen
        ...
        kills_by_me = AnalyticsService.get_kills_by_me(player_lower, date_filter, tuple(kill_params))
        deaths_total = AnalyticsService.get_deaths_total(player_lower, date_filter, tuple(death_params))
        suicides = AnalyticsService.get_suicides(player_lower, date_filter, tuple(suicide_params))
        ...
        kills_detail = AnalyticsService.get_kills_detail(player_lower, date_filter, tuple(kill_detail_params))
        deaths_detail = AnalyticsService.get_deaths_detail(player_lower, date_filter, tuple(death_detail_params))
...
def get_recent_kill_events(start_date=None, end_date=None, entity_filters=None):
    ...
    try:
        ...
        recent_res = AnalyticsService.get_recent_events(tuple(all_params), date_filter)
...
@lru_cache(maxsize=32)
def get_leaderboards_cached(start_date_str, end_date_str, entity_filters_tuple):
    """
    Caching-Variante für get_leaderboards. Die Parameter werden als Strings/Tuples übergeben, damit lru_cache funktioniert.
    """
    entity_filters = dict(entity_filters_tuple) if entity_filters_tuple else None
    start_date = datetime.strptime(start_date_str, "%Y-%m-%d") if start_date_str else None
    end_date = datetime.strptime(end_date_str, "%Y-%m-%d") if end_date_str else None
    return get_leaderboards(start_date, end_date, entity_filters)
...
def get_leaderboards(start_date=None, end_date=None, entity_filters=None):
    ...
    try:
        ...
        kill_params = [player_lower, player_lower] + date_params
        all_kills = AnalyticsService.get_all_kills(player_lower, date_filter, tuple(kill_params))
        death_params = [player_lower, player_lower] + date_params
        all_deaths = AnalyticsService.get_all_deaths(player_lower, date_filter, tuple(death_params))
