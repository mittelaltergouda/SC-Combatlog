...
import webbrowser
import logging
from core import update_checker  # Neues Modul für Updates
from logging.handlers import RotatingFileHandler
from dataclasses import dataclass
from typing import List, Tuple
from core import config
from data import database, database_service
from log_processing import log_processor
from analytics import stats
from log_processing.watchdog_handler import start_watchdog
from datetime import datetime
from core.ui_constants import Colors, Fonts, WindowSettings, RefreshSettings
...
class GriefingCounterApp(tk.Tk):
    ...
    def apply_date_filter(self):
        """Apply date filter and refresh data (zentral über FilterService)."""
        ...
        try:
            self.active_start_date = FilterService.validate_date(start_date)
            self.active_end_date = FilterService.validate_date(end_date)
    ...
    def load_data_with_filters(self, start_date, end_date):
        """Load data with date filters (zentral über FilterService)."""
        try:
            entity_filters = FilterService.build_entity_filter_dict(self.entity_filters)
    ...
    def apply_entity_filter(self):
        """Wendet die ausgewählten Entity-Filter an und aktualisiert die Anzeige (zentral über FilterService)."""
    ...
    def load_data_with_all_filters(self):
        """Lädt Daten mit allen aktiven Filtern (Datum und Entity, zentral über FilterService)."""
        try:
            entity_filters = FilterService.build_entity_filter_dict(self.entity_filters)
    ...
    def on_clear_appdata(self):
        ...
        try:
            # Lösche AppData über Service
            ok = database_service.DatabaseService.reset_appdata()
            if ok:
                messagebox.showinfo(
                    "AppData gelöscht",
                ...
                messagebox.showerror("Fehler", "Fehler beim Löschen der Daten.")
        except Exception as e:
